fn main() {
    ...
    let sum = get_value_for_schematic(inputs);
    println!("{}", sum);
}

fn get_value_for_schematic(input: &str) -> u32 {
...
fn does_symbol_exist_on_line_in_range(line: &str, start_pos: u32, end_pos: u32) -> bool {
    ...
    for char in trimmed_string.to_string().chars() {
        if char != ' ' && char.is_alphanumeric() == false {
            ret_val = true
        }
    }
...
...$.*....
.664.598..";
        assert_eq!(get_value_for_schematic(test_string), 4361);
    }
    #[test]
    fn find_number() {
